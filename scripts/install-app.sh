...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.normies.desktop"
    if pgrep -x "Normies" >/dev/null 2>&1; then
        info "Quitting Normies..."
        ...
        if pgrep -x "Normies" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Normies" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Normies has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Normies'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.normies/app"
    WRAPPER_PATH="$INSTALL_DIR/normies"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Normies-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Normies.*AppImage" >/dev/null 2>&1; then
        info "Stopping Normies..."
        pkill -f "Normies.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# Normies launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.normies/app/Normies-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@normies"
ELECTRON_CACHE_ALT="$HOME/.cache/@normies"
...
# AppImage creates a new /tmp/.mount_Normies-XXXX each launch, so any cached path is stale
for cache_dir in "$ELECTRON_CACHE" "$ELECTRON_CACHE_ALT"; do
    if [ -d "$cache_dir" ] && grep -rq '/tmp/\.mount_Normies' "$cache_dir" 2>/dev/null; then
        rm -rf "$cache_dir"
    fi
done
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Normies-x64.AppImage"
