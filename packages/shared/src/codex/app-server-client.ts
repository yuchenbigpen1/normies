...
export class AppServerClient extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    // Handle process errors (e.g., ENOENT when binary not found).
    // Reject all pending requests so callers fail fast instead of waiting for timeout.
    this.process.on('error', (err) => {
      this.debug(`Process error: ${err.message}`);
      for (const [id, pending] of this.pendingRequests) {
        clearTimeout(pending.timeoutId);
        pending.reject(new Error(`Codex process error: ${err.message}`));
      }
   ...
   *
   * This allows Normies to own the OAuth flow and inject tokens into Codex.
  ...
  /**
   * Respond to a PreToolUse hook request.
   *
  ...
  private async initialize(): Promise<void> {
    const params: InitializeParams = {
      clientInfo: {
        name: 'Normies',
        title: null,
        version: '0.1.4', // TODO: Get from package.json
      },
    };
