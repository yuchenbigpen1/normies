...
### The Stuck Non-Technical Person

Anyone who isn't an engineer but wants to get technical things done. They might run a Shopify store, manage a department, create content, run a side project, or just have an idea they want to bring to life. They're capable, motivated people — they just can't write code.
...
**We are Claude Code for people who aren't engineers. Not another tool promising to eliminate complexity — a technical teammate that helps you navigate it honestly and get to a working solution.**
...
**Core thesis:** Non-technical people don't need another code generator. They need a technical teammate that helps them think before they build, honestly assesses complexity, manages context while they build, and catches mistakes as they go — all in language they understand.
...
| Writing Plans | obra/superpowers | Breaks work into bite-sized tasks with file paths, acceptance criteria, TDD emphasis. Offers subagent or parallel execution. | Add Excalidraw architecture diagram generation. Add plain language summaries. Add "this part will be hard" flags. |
...
- Auto-generate Excalidraw architecture diagrams during plan creation
...
Superpowers creates tasks as markdown entries in a plan file and executes them sequentially via subagents in a terminal. This works for developers watching CLI output. It does NOT work for non-technical users.
...
**Why not auto-execute?** For non-technical users, understanding is more important than speed. Auto-execute can be added later as a power-user toggle. Starting restrictive and loosening is easier than the reverse.
...
1. **Always-visible architecture diagram** — The Excalidraw architecture visualization isn't decorative. It's the user's map. Accessible via sidebar button, updates after each task completes, color-coded by status (todo → in progress → done). The architecture diagram is view-only and focused on comprehension, not task navigation. This is the primary re-entry tool — user comes back after 3 days, opens the diagram, sees project state at a glance.
...
- Architecture diagram in sidebar panel (triggered by button, view-only)
...
**Implementation:** Modified system prompt for Craft Agents' existing Explore mode. We add the "Don't Build" gate, plain language communication rules, complexity honesty prompts, and Excalidraw architecture diagram generation to the Explore system prompt. No new mode needed — we're editing what's already there.

**Powered by:** Modified `superpowers:brainstorming` + `superpowers:writing-plans` + "Don't Build" gate + complexity honesty prompts + Excalidraw architecture diagram generation
...
- Architecture/workflow design + auto-generated Excalidraw architecture diagram
...
Plan generated (still within Explore conversation, validated in chunks)
  → Architecture/workflow design + Excalidraw architecture diagram + task breakdown
