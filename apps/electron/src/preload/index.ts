...
const api: ElectronAPI = {
  ...
  onMenuToggleSessionList: (callback: () => void) => {
    const handler = () => callback()
    ipcRenderer.on(IPC_CHANNELS.MENU_TOGGLE_SESSION_LIST, handler)
    return () => ipcRenderer.removeListener(IPC_CHANNELS.MENU_TOGGLE_SESSION_LIST, handler)
  },
  ...
  createSource: (workspaceId: string, config: Partial<import('@normies/shared/sources').FolderSourceConfig>) =>
  ...
  onSourcesChanged: (callback: (sources: import('@normies/shared/sources').LoadedSource[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, sources: import('@normies/shared/sources').LoadedSource[]) => {
      callback(sources)
    }
  ...
  onSkillsChanged: (callback: (skills: import('@normies/shared/skills').LoadedSkill[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, skills: import('@normies/shared/skills').LoadedSkill[]) => {
      callback(skills)
    }
  ...
  onAppThemeChange: (callback: (theme: import('@normies/shared/config').ThemeOverrides | null) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, theme: import('@normies/shared/config').ThemeOverrides | null) => {
      callback(theme)
    }
  ...
  // Menu actions (for unified Normies menu)
