...
import { X, MoreHorizontal, Pencil, Trash2, Star, ChevronDown, ChevronRight, CheckCircle2, RefreshCcw } from 'lucide-react'
import { Spinner, FullscreenOverlayBase } from '@normies/ui'
import { useSetAtom } from 'jotai'
import { fullscreenOverlayOpenAtom } from '@/atoms/overlay'
import { motion, AnimatePresence } from 'motion/react'
import type { LlmConnectionWithStatus, LlmConnection, ThinkingLevel, WorkspaceSettings, Workspace } from '../../../shared/types'
import { DEFAULT_THINKING_LEVEL, THINKING_LEVELS } from '@normies/shared/agent/thinking-levels'
...
function ConnectionRow({ connection, isLastConnection, onRenameClick, onDelete, onSetDefault, onValidate, onReauthenticate, validationState, validationError }: ConnectionRowProps) {
  ...
  const getDescription = () => {
    ...
    // Base URL for non-OAuth connections
  ...
  const labelText = connection.isDefault
...
function WorkspaceOverrideCard({ workspace, llmConnections, onSettingsChange }: WorkspaceOverrideCardProps) {
  ...
  const hasOverrides = settings && (settings.model || settings.thinkingLevel)

  // Get display values
  const currentModel = settings?.model || 'global'
  const currentThinking = settings?.thinkingLevel || 'global'

  // Derive workspace's effective connection (default)
...
export default function AiSettingsPage() {
  ...
  const handleRenameSubmit = useCallback(async () => {
    ...
    try {
      // Find the full connection from our list, update name, and save
      const connection = llmConnections.find(c => c.slug === renamingConnection.slug)
      if (connection) {
        const { isAuthenticated: _a, authError: _b, isDefault: _c, ...connectionData } = connection
        await window.electronAPI.saveLlmConnection({ ...connectionData, name: trimmedName } as LlmConnection)
        refreshLlmConnections?.()
      }
    } catch (error) {
    ...
    setRenameValue('')
  }, [renamingConnection, renameValue, llmConnections, refreshLlmConnections])
  ...
  const handleReauthenticateConnection = useCallback((connection: LlmConnectionWithStatus) => {
    ...
    if (connection.authType === 'oauth') {
      apiSetupOnboarding.handleStartOAuth()
    }
  ...
  const handleValidateConnection = useCallback(async (connection: LlmConnectionWithStatus) => {
    if (!window.electronAPI) return
    ...
    try {
      // testLlmConnection takes a full connection object
      const { isAuthenticated: _a, authError: _b, isDefault: _c, ...connectionData } = connection
      const result = await window.electronAPI.testLlmConnection(connectionData as LlmConnection)
  ...
  const handleDefaultModelChange = useCallback(async (model: string) => {
    ...
    // Update defaultModel on the connection, then save
    const { isAuthenticated: _a, authError: _b, isDefault: _c, ...connectionData } = defaultConnection
    await window.electronAPI.saveLlmConnection({ ...connectionData, defaultModel: model } as LlmConnection)
  ...
  return (
    <div className="h-full flex flex-col">
      <PanelHeader title="AI" leftActions={<NavigationButtons />} actions={<HeaderMenu route={routes.view.settings('ai')} />} />
                  ...
                  {llmConnections.length === 0 ? (
                    ...
                    [...llmConnections]
                      ...
                      <ConnectionRow
                        ...
                        onValidate={() => handleValidateConnection(conn)}
