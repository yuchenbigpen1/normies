...
import { checkDependencies, buildWaveContext } from "@/utils/wave-utils"
              ...
              <div className={cn(CHAT_LAYOUT.maxWidth, "mx-auto", CHAT_LAYOUT.containerPadding, CHAT_LAYOUT.messageSpacing, "min-w-0")}>
                ...
                <AnimatePresence mode="wait" initial={false}>
                  ...
                  {!compactMode && session?.taskIndex != null && session?.projectId && (() => {
                    ...
                    // Dependency enforcement: check if all prerequisite tasks are done
                    const siblingTasks = Array.from(sessionMetaMap.values())
                      .filter(s => s.projectId === session.projectId && s.taskIndex != null)
                    const depCheck = checkDependencies(session as any, siblingTasks as any)
                    const canStart = isTodo && !isStarting && depCheck.met
                    ...
                    const buildExecutionContext = () => {
                      ...
                      if (session.projectStatePath) {
                        parts.push(`\n### Project State\nRead the project state file for context about what other tasks have accomplished: ${session.projectStatePath}`)
                      }

                      // Step/wave context: tell the agent about parallel siblings
                      if (session.wave != null) {
                        const allProjectTasks = Array.from(sessionMetaMap.values())
                          .filter(s => s.projectId === session.projectId && s.taskIndex != null)
                        // Look up projectSteps from parent session
                        const parentMeta = Array.from(sessionMetaMap.values())
                          .find(s => s.projectId === session.projectId && s.taskIndex == null)
                        const waveCtx = buildWaveContext(session as any, allProjectTasks as any, parentMeta?.projectSteps)
                        if (waveCtx) parts.push(waveCtx)
                      }
                    ...
                    return (
                      <div className={`${hasStarted ? 'py-3' : 'py-6'} select-none transition-all duration-500 ${isStarting ? 'opacity-60' : 'opacity-100'}`}>
                        ...
                        />
                        {/* Dependency gate: show waiting message when deps aren't met */}
                        {isTodo && !depCheck.met && (
                          <div className="flex items-center gap-2 px-4 pt-2 text-xs text-muted-foreground">
                            <span className="opacity-60">Waiting for {depCheck.unmetTasks.map(t => t.name ? `${t.name}` : `Task ${t.index + 1}`).join(', ')} to finish first</span>
                          </div>
                        )}
                      </div>
                    )
                  ...
                  {turns.map((turn, index) => {
                    ...
                    return (
                      ...
                      <TurnCard
                        ...
                        onReview={session && onSendMessage ? () => {
                          onSendMessage('Review our conversation so far.', [], [])
                        } : undefined}
                      />
                      </div>
                    )
